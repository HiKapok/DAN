...
tf.app.flags.DEFINE_string(
    'checkpoint_exclude_scopes', 'dan/predict_face, dan/predict_cascade_bbox, dan/predict_cascade_cls, dan/shift_upsampled, dan/additional_layers, dan/l2_norm_layer_3, dan/l2_norm_layer_4, dan/l2_norm_layer_5, dan/lfpn',
...
def dan_model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope(params['model_scope'], default_name=None, values=[features], reuse=tf.AUTO_REUSE):
        ...
        final_location_pred, _ = backbone.get_predict_module(feature_layers, [1] * len(feature_layers),
                                        [3] + [1] * (len(feature_layers) - 1), all_num_anchors_depth, name='predict_cascade_bbox')
        _, final_cls_pred = backbone.get_predict_module(shift_feayure_layers, [1] * len(shift_feayure_layers),
                                        [3] + [1] * (len(shift_feayure_layers) - 1), all_num_anchors_depth, name='predict_cascade_cls')
