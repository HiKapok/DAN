    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope(FLAGS.model_scope, default_name=None, values=[features], reuse=tf.AUTO_REUSE):
            ...
            location_pred, cls_pred = backbone.get_density_se_predict_module(feature_layers, [1] * len(feature_layers),
                                        [1] + [1] * (len(feature_layers) - 1), anchors_depth, offsets=[[-0.3, -0.3], [-0.3, 0.3], [0.3, -0.3], [0.3, 0.3]] + [None] * (len(feature_layers) - density_ratio), var_name_ind=[0, 0, 0] + list(range(len(feature_layers) - density_ratio + 1)), name='predict_face')
            if FLAGS.data_format == 'channels_first':
            ...
            final_location_pred, final_cls_pred = \
                backbone.get_density_se_predict_module(feature_layers[0:FLAGS.cascaded_skip], \
                                            ...
                                            [3, 3, 3, 3] + [1] * (len(feature_layers) - density_ratio), \
                                            anchors_depth, \
                                            offsets=[[-0.3, -0.3], [-0.3, 0.3], [0.3, -0.3], [0.3, 0.3]] +  \
                                                    [None] * (len(feature_layers) - density_ratio), \
                                            var_name_ind=[0, 0, 0] + list(range(len(feature_layers) - density_ratio + 1)), \
            ...
            for index, event in enumerate(event_list):
                ...
                for num, file in enumerate(filelist):
                    ...
                    if num % FLAGS.log_every_n_steps == 0:
                        ...
                        imsave(os.path.join('./dan_debug_density_v2/{}.jpg').format(im_name), img_to_draw)
