...
def anchor_routing(decoded_bbox, gt_bboxes, gt_lables, easy_mask, feat_height, feat_width, feat_strides, all_num_anchors_depth, num_anchors_per_layer, threshold_per_layer, skip_flags=None):
    if skip_flags is None: skip_flags = [False] * len(all_num_anchors_depth)
...
def dan_model_fn(features, labels, mode, params):
    ...
    with tf.variable_scope(params['model_scope'], default_name=None, values=[features], reuse=tf.AUTO_REUSE):
        ...
        final_location_pred, final_cls_pred = backbone.get_se_predict_module(feature_layers[:params['cascaded_skip']], [1] * len(feature_layers),
        ...
        with tf.control_dependencies([cls_pred, location_pred]):
            with tf.name_scope('post_forward'):
                ...
                final_mask, final_loc_targets = anchor_routing(bboxes_pred, matched_gt, tf.to_float(cls_targets > 0),
                                            ...
                                            feat_height_list, feat_width_list, [4, 8, 16, 32, 64, 128], all_num_anchors_depth, num_anchors_per_layer, [0.4, 0.5, 0.6, 0.7, 0.8, 0.9], skip_flags=[False] * params['cascaded_skip'] + [True] * (6 - params['cascaded_skip']))
    ...
    for trainable_var in tf.trainable_variables():
        if 'predict_cascade/loc_' not in trainable_var.name:
    ...
    total_loss = tf.add(cross_entropy + loc_loss + final_cross_entropy, tf.multiply(params['weight_decay'], tf.add_n(l2_loss_vars), name='l2_loss'), name='total_loss')
...
def main(_):
    ...
    dan_detector = tf.estimator.Estimator(
        ...
        params={
            ...
            'cascaded_skip': FLAGS.cascaded_skip,
