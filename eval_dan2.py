...
tf.app.flags.DEFINE_string(
    'checkpoint_path', './dan_shift_cls_logs/',
...
def main(_):
    with tf.Graph().as_default():
        ...
        with tf.variable_scope(FLAGS.model_scope, default_name=None, values=[features], reuse=tf.AUTO_REUSE):
            ...
            shift_feayure_layers = backbone.get_shift_upsampled_feature(feature_layers)

            if FLAGS.data_format == 'channels_first':
                shift_feayure_layers = [tf.transpose(layer, [0, 2, 3, 1]) for layer in shift_feayure_layers]
            croped_shift_feayure_layers = list()
            for ind, feat in enumerate(shift_feayure_layers):
                croped_shift_feayure_layers.append(tf.image.crop_to_bounding_box(feat, 0, 0, all_layer_shapes[ind][0], all_layer_shapes[ind][1]))
            if FLAGS.data_format == 'channels_first':
                shift_feayure_layers = [tf.transpose(layer, [0, 1, 2, 3]) for layer in croped_shift_feayure_layers]
            else:
                shift_feayure_layers = croped_shift_feayure_layers

            #feature_layers[0] = tf.Print(feature_layers[0], [tf.shape(_) for _ in feature_layers])
            #shift_feayure_layers[0] = tf.Print(shift_feayure_layers[0], [tf.shape(_) for _ in shift_feayure_layers])
            ...
            location_pred, cls_pred = backbone.get_predict_module(feature_layers, [1] * len(feature_layers),
            ...
            final_location_pred, _ = backbone.get_predict_module(feature_layers, [1] * len(feature_layers),
                                        [3] + [1] * (len(feature_layers) - 1), anchors_depth, name='predict_cascade_bbox')
            _, final_cls_pred = backbone.get_predict_module(shift_feayure_layers, [1] * len(shift_feayure_layers),
                                        [3] + [1] * (len(shift_feayure_layers) - 1), anchors_depth, name='predict_cascade_cls')
            if FLAGS.data_format == 'channels_first':
