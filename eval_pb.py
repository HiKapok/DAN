...
tf.app.flags.DEFINE_string(
    'checkpoint_path', './pyramid_box_logs',
...
def main(_):
    with tf.Graph().as_default():
        ...
        features, output_shape = pb_preprocessing.preprocess_for_eval(image_input, target_shape, data_format=FLAGS.data_format, output_rgb=False)
        ...
        with tf.variable_scope(FLAGS.model_scope, default_name=None, values=[features], reuse=tf.AUTO_REUSE):
            backbone = pb_net.VGG16Backbone(FLAGS.data_format)
            ...
            location_pred, cls_pred = backbone.get_predict_module(feature_layers, [1] + [3] * (len(feature_layers) - 1),
                                            [3] + [1] * (len(feature_layers) - 1), anchors_depth, name='predict_face')
