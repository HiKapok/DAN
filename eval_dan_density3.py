...
tf.app.flags.DEFINE_integer(
    'cascaded_skip', 9, 'Number of skip layers in the cascaded stage.')
tf.app.flags.DEFINE_integer(
...
def main(_):
    with tf.Graph().as_default():
        ...
        all_anchor_scales = [(16.,), (16.,), (16.,), (16.,), (32.,), (64.,), (128.,), (256.,), (512.,)]
        all_extra_scales = [(), (), (), (), (), (), (), (), ()]
        #all_anchor_ratios = [(1.,), (1.,), (1.,), (1.,), (1.,), (1.,), (1.,), (1.,), (1.,)]
        all_anchor_ratios = [(0.8,), (0.8,), (0.8,), (0.8,), (0.8,), (0.8,), (0.8,), (0.8,), (0.8,)]
        all_layer_strides = [4, 4, 4, 4, 8, 16, 32, 64, 128]
        offset_list = [(0.2,0.2), (0.2,0.8), (0.8,0.2), (0.8,0.8), 0.5, 0.5, 0.5, 0.5, 0.5]
        with tf.variable_scope(FLAGS.model_scope, default_name=None, values=[features], reuse=tf.AUTO_REUSE):
            ...
            shift_feature_maps = []
            density_ratio = 4
            for feat_ind in range(1):
                test_offsets = [[-0.3, -0.3], [-0.3, 0.3], [0.3, -0.3], [0.3, 0.3]]
                if FLAGS.data_format == 'channels_first':
                    feats = tf.transpose(feature_layers[feat_ind], [0, 2, 3, 1])
                else:
                    feats = feature_layers[feat_ind]
                feat_size = tf.shape(feats)[1:-1]
                for ind, (offset_h, offset_w) in enumerate(test_offsets):
                    croped_feats = tf.image.crop_and_resize(feats, tf.expand_dims(tf.stack([offset_h/tf.to_float(feat_size[0]), offset_w/tf.to_float(feat_size[1]), 1. + offset_h/tf.to_float(feat_size[0]), 1. + offset_w/tf.to_float(feat_size[1])], axis=-1), axis=0), tf.constant([0]), feat_size, method='bilinear', extrapolation_value=0)
                    if FLAGS.data_format == 'channels_first':
                        croped_feats = tf.transpose(croped_feats, [0, 3, 1, 2])
                    shift_feature_maps.append(croped_feats)
            feature_layers = shift_feature_maps + feature_layers[1:]
            ...
            post_feature_layers = feature_layers[(density_ratio-1):]
            location_pred_post, cls_pred_post = backbone.get_se_predict_module(post_feature_layers, [1] * len(post_feature_layers),
                                        [1] + [1] * (len(post_feature_layers) - 1), anchors_depth[(density_ratio-1):], name='predict_face')
            location_pred_pre = []
            cls_pred_pre = []
            for density_ind in range(density_ratio - 1):
                _location_pred, _cls_pred = backbone.get_se_predict_module([feature_layers[density_ind]], [1], [1], [anchors_depth[density_ind]], name='predict_face', reuse=True)
                location_pred_pre.append(_location_pred)
                cls_pred_pre.append(_cls_pred)
            cls_pred = cls_pred_pre + cls_pred_post
            location_pred = location_pred_pre + location_pred_post
            if FLAGS.data_format == 'channels_first':
            ...
            final_location_pred_post, final_cls_pred_post = backbone.get_se_predict_module(post_feature_layers[0:(FLAGS.cascaded_skip - (density_ratio - 1))], [1] * len(post_feature_layers),
                                        [3] + [1] * (len(post_feature_layers) - 1), anchors_depth[(density_ratio-1):], name='predict_cascade')
            final_location_pred_pre = []
            final_cls_pred_pre = []
            for density_ind in range(density_ratio - 1):
                _final_location_pred, _final_cls_pred = backbone.get_se_predict_module([feature_layers[density_ind]], [1], [3], [anchors_depth[density_ind]], name='predict_cascade', reuse=True)
                final_location_pred_pre.append(_final_location_pred)
                final_cls_pred_pre.append(_final_cls_pred)
            final_cls_pred = final_cls_pred_pre + final_cls_pred_post
            final_location_pred = final_location_pred_pre + final_location_pred_post
        ...
        with tf.device('/cpu:0'):
            ...
            cls_pred_list_stage1 = tf.split(cls_pred, tf.stack(num_anchors_per_layer), axis=0, name='split_cls_pred_stage1')

            num_anchors_per_cascaded_layer = tf.stack(num_anchors_per_layer[0:FLAGS.cascaded_skip])
            if FLAGS.data_format == 'channels_first':
            ...
            decoded_bbox_list = tf.split(bboxes_pred, tf.stack(num_anchors_per_layer), axis=0, name='split_decoded_bbox')
            gt_bboxes_list = tf.split(final_location_pred / tf.expand_dims(tf.constant([10., 10., 5., 5.], dtype=tf.float32) * 1., axis=0), num_anchors_per_cascaded_layer, axis=0, name='split_gt_bboxes')
            gt_lables_list = tf.split(final_cls_pred, num_anchors_per_cascaded_layer, axis=0, name='split_gt_lables')
            easy_mask_list = tf.split(tf.to_int32(cls_pred > 0.03), tf.stack(num_anchors_per_layer), axis=0, name='split_easy_mask')
            mask_out_list = []
            decode_out_list = []
            feat_strides = [4, 4, 4, 4, 8, 16, 32, 64, 128]
            skip_flags = [False] * FLAGS.cascaded_skip + [True] * (len(feat_strides) - FLAGS.cascaded_skip)
            for ind in range(len(anchors_depth)):
                if skip_flags[ind]: continue
            ...
            cls_pred = tf.concat([cls_pred] + cls_pred_list_stage1[FLAGS.cascaded_skip:], axis=0)
            ...
            for index, event in enumerate(event_list):
                ...
                for num, file in enumerate(filelist):
                    ...
                    if num % FLAGS.log_every_n_steps == 0:
                        ...
                        imsave(os.path.join('./dan_debug_density/{}.jpg').format(im_name), img_to_draw)
