...
def sfd_random_sample_patch(image, labels, bboxes, ratio_list=[0.1, 0.3, 0.5, 0.7, 0.9, 1.], name=None):
  '''sfd_random_sample_patch.
  select one min_iou
  ...
  with tf.name_scope('sfd_random_sample_patch'):
...
def sfd_random_expand(image, bboxes, ratio=2., name=None):
  with tf.name_scope('sfd_random_expand'):
    image = tf.convert_to_tensor(image, name='image')
...
# # ssd augumentation
# def sfd_random_sample_patch_wrapper(image, labels, bboxes):
#   with tf.name_scope('sfd_random_sample_patch_wrapper'):
...
#                       lambda: sfd_random_expand(orgi_image, orgi_bboxes, tf.random_uniform([1], minval=1.1, maxval=4., dtype=tf.float32)[0]))
#       # Distort image and bounding boxes.
#       random_sample_image, labels, bboxes = sfd_random_sample_patch(image, orgi_labels, bboxes, ratio_list=[-0.1, 0.1, 0.3, 0.5, 0.7, 0.9, 1.])
...
def sfd_random_sample_patch_wrapper(image, bboxes):
  with tf.name_scope('sfd_random_sample_patch_wrapper'):
...
def preprocess_for_train(image, bboxes, out_shape, data_format='channels_first', scope='sfd_preprocessing_train', output_rgb=True):
  ...
  """
  with tf.name_scope(scope, 'sfd_preprocessing_train', [image, bboxes]):
    if image.get_shape().ndims != 3:
      raise ValueError('Input must be of size [height, width, C>0]')
    ...
    random_sample_image, bboxes = sfd_random_sample_patch_wrapper(distort_image, bboxes)
...
def preprocess_for_eval(image, out_shape, data_format='channels_first', scope='sfd_preprocessing_eval', output_rgb=True):
  ...
  Returns:
    A preprocessed image.
  """
  with tf.name_scope(scope, 'sfd_preprocessing_eval', [image]):
...
def preprocess_image(image, bboxes, out_shape, is_training=False, data_format='channels_first', output_rgb=True):
  ...
  """
  if is_training:
    return preprocess_for_train(image, bboxes, out_shape, data_format=data_format, output_rgb=output_rgb)
  else:
