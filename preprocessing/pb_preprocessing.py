...
def sfd_random_sample(image, bboxes, out_shape):
  with tf.name_scope('sfd_random_sample', [image, bboxes]):
    random_sample_image, bboxes = sfd_random_sample_patch_wrapper(image, bboxes)
...
def preprocess_for_train(image, bboxes, out_shape, anchor_scales, data_format='channels_first', scope='pyramid_box_preprocessing_train', output_rgb=True):
  ...
  """
  with tf.name_scope(scope, 'pyramid_box_preprocessing_train', [image, bboxes]):
    if image.get_shape().ndims != 3:
      raise ValueError('Input must be of size [height, width, C>0]')
    ...
    random_sample_image, bboxes = tf.cond(tf.random_uniform([], 0, 1.0) < 0.5, lambda: sfd_random_sample(distort_image, bboxes, out_shape), \
...
def preprocess_for_eval(image, out_shape, data_format='channels_first', scope='pyramid_box_preprocessing_eval', output_rgb=True):
  ...
  Returns:
    A preprocessed image.
  """
  with tf.name_scope(scope, 'pyramid_box_preprocessing_eval', [image]):
