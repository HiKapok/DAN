...
class AnchorEncoder(object):
    ...
    def encode_anchors(self, bboxes, anchors_ymin, anchors_xmin, anchors_ymax, anchors_xmax, inside_mask, match_mining=False, debug=False):
        ...
        with tf.name_scope('encode_anchors'):
            ...
            bboxes = tf.cond(tf.shape(bboxes)[0] < 1, lambda: tf.constant([[0., 0., 1., 1.]]), lambda: bboxes)
            ...
            matched_indices = tf.clip_by_value(matched_gt, 0, tf.int32.max)
            #matched_indices = tf.Print(matched_indices, [tf.boolean_mask(matched_indices, matched_indices>0)], summarize=1000)

            gt_labels =  tf.cast(matched_gt_mask, tf.int64)
            # set those ignored positions to -1
            gt_labels = gt_labels + (-1 * tf.to_int64(matched_gt < -1))

            #gt_labels = tf.Print(gt_labels, [tf.count_nonzero(tf.equal(gt_labels, 0)), tf.count_nonzero(gt_labels>0)])
            #bboxes = tf.Print(bboxes, [tf.shape(bboxes)[0]])
            matched_gt_bbox = tf.gather(bboxes, matched_indices, axis=0)
            gt_ymin, gt_xmin, gt_ymax, gt_xmax = tf.unstack(matched_gt_bbox, 4, axis=-1)
            ...
            #gt_targets = tf.Print(gt_targets, [gt_targets], summarize=1000)
            return gt_targets, gt_labels, gt_scores, matched_gt_bbox * tf.expand_dims(tf.to_float(matched_gt_mask), -1)

    def encode_pa_anchors(self, bboxes, anchors_ymin, anchors_xmin, anchors_ymax, anchors_xmax, inside_mask, ignore_threshold, positive_threshold, match_mining=True, scale=1., debug=False):
        '''encode anchors with ground truth on the fly
        ...
        with tf.name_scope('encode_anchors'):
            #image_height, image_width = tf.to_float(image_shape[0]), tf.to_float(image_shape[1])
            anchor_cy, anchor_cx, anchor_h, anchor_w = self.point2center(anchors_ymin, anchors_xmin, anchors_ymax, anchors_xmax)
            # downsample the anchors, and then do matching
            all_anchors = tf.stack(self.center2point(anchor_cy, anchor_cx, anchor_h / scale, anchor_w / scale), axis=-1)
            # all_anchors = tf.stack([tf.maximum(gt_ymin - gt_h / scale, 0.),
            #                         tf.maximum(gt_xmin - gt_w / scale, 0.),
            #                         tf.minimum(gt_ymax + gt_h / scale, image_height - 1.),
            #                         tf.minimum(gt_xmax + gt_w / scale, image_width - 1.)], axis=-1)
            bboxes = tf.cond(tf.shape(bboxes)[0] < 1, lambda: tf.constant([[0., 0., 1., 1.]]), lambda: bboxes)
            overlap_matrix = iou_matrix(all_anchors, bboxes) * tf.cast(tf.expand_dims(inside_mask, 1), tf.float32)
            #overlap_matrix = tf.Print(overlap_matrix, [tf.shape(overlap_matrix)])
            #matched_gt, gt_scores = custom_op.small_mining_match(overlap_matrix, 0., self._rpn_negtive_threshold, self._rpn_positive_threshold, 5, 0.35)
            ...
            gt_cy = (gt_cy - anchor_cy) / anchor_h / self._prior_scaling[0]
            gt_cx = (gt_cx - anchor_cx) / anchor_w / self._prior_scaling[1]
            gt_h = tf.log(gt_h * scale / anchor_h) / self._prior_scaling[2]
            gt_w = tf.log(gt_w * scale / anchor_w) / self._prior_scaling[3]
            # now gt_localizations is our regression object, but also maybe chaos at those non-positive positions
            if debug:
                gt_targets = tf.stack([anchors_ymin, anchors_xmin, anchors_ymax, anchors_xmax], axis=-1)
            else:
